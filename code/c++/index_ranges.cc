...
void index(const string & datapath, const string & dbpath)
{
    ...
    const size_t FIELD_MEASUREMENTS = 6;
    const size_t FIELD_DESCRIPTION = 8;
    const size_t FIELD_DATE_MADE = 10;
	...
	/* Start of example code.
        # parse the two values we need*/
       	string & measurements = fields.at(FIELD_MEASUREMENTS);
        doc.add_value(0,  numbers_from_string(measurements));
        string & date_made = fields.at(FIELD_DATE_MADE);
        doc.add_value(1, numbers_from_string(date_made));
	// End of example code.
