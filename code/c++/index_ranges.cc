...
void index(const string & datapath, const string & dbpath)
{
    ...
    const size_t FIELD_DATE_MADE = 4;
    const size_t FIELD_MEASUREMENTS = 7;
	...
	fields.at(FIELD_DATE_MADE) != "DATE_MADE" ||
	fields.at(FIELD_MEASUREMENTS) != "MEASUREMENTS" ||
	...
	const string & title = fields.at(FIELD_TITLE);
        const string & measurements = fields.at(FIELD_MEASUREMENTS);
        const string & date_made = fields.at(FIELD_DATE_MADE);
...
// Start of example code.
	...
	if (max_number_in_string(measurements, &n)) {
	    doc.add_value(0, Xapian::sortable_serialise(n));
	}

	if (first_number_in_string(date_made, &n)) {
            doc.add_value(1, Xapian::sortable_serialise(n));
	}
// End of example code.
